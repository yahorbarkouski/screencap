...
function slotTitle(slot: DaylineSlot, mode: DaylineViewMode): string {
	...
	if (mode === "categories")
		return `${time} · ${count} captures · ${slot.category}`;
	if (mode === "apps")
		return `${time} · ${count} captures · ${slot.appName ?? "Unknown"}`;
	if (slot.addiction)
		return `${time} · ${count} captures · Addiction: ${slot.addiction}`;
	return `${time} · ${count} captures · Non-addiction`;
}
